                            ...
                            <form  method="post">
                                ...
                                <select name="vxl_chargeMethod" id="vxl_domain" rows="8" style="font-family:Courier New;" value ="<?php echo get_option('vxl_trancherAtParagraphNo'); ?>">
                                    <option value="chargeByTag" <?php
                                            ...
                                            ?> >  
                                            <?php _e('Articles tagged Premium, default to free', $this->plugin->name); ?></option>
                                    <option value="chargeEverything"  <?php
                                            if (get_option('vxl_chargeMethod') == 'chargeEverything') {
                                                print ' selected ';
                                            }
                                            ?> >    
<?php _e('Charge for all articles', $this->plugin->name); ?></option>
                                    <option value="chargeContentURL"  <?php
if (get_option('vxl_chargeMethod') == 'chargeContentURL') {
    print ' selected ';
}
?> >    
<?php _e('Charge content whose URL ends with -vxl', $this->plugin->name); ?></option>
                                ...
                                <h3 class="hndle"><?php _e('Shortening content before payment', $this->plugin->name); ?></h3>
                                        ...
                                        <?php _e('When a user arrives at paid content, the content is chopped awaiting the vxl payment process to complete. By default, widescribe will chop the article when detecting a ::widescribe:: in the string. If this is not present, please select approximately how many words the user will be allowed to read if his account is empty. <br> ', $this->plugin->name); ?>
                                ...
                                <select name="vxl_trancherAt" id="vxl_domain" rows="8" style="font-family:Courier New;" value ="<?php echo get_option('vxl_trancherAtParagraphNo'); ?>">
                                    <option value="trancher_at_token" <?php
                                            if (get_option('vxl_trancherAt') == 'trancher_at_token') {
                                                print ' selected ';
                                            }
                                        ?> >  
                                    <?php _e('Cut when ::vxl:: appearing in text', $this->plugin->name); ?></option>
                                    <option value="trancher_after_heading"  <?php
                                            if (get_option('vxl_trancherAt') == 'trancher_after_heading') {
                                                print ' selected ';
                                            }
                                            ?> >    
                                    <?php _e('Cut after heading', $this->plugin->name); ?></option>
                                    <option value="trancher_after_ingres"  <?php
                                        if (get_option('vxl_trancherAt') == 'trancher_after_ingres') {
                                            print ' selected ';
                                        }
                                        ?> >   
                                        <?php _e('Cut after ingres', $this->plugin->name); ?></option>
                                    <option value="trancher_after_150_words"  <?php
                                    if (get_option('vxl_trancherAt') == 'trancher_after_150_words') {
                                        print ' selected ';
                                    }
                                    ?> >   
<?php _e('Cut after approx 150 words', $this->plugin->name); ?></option>

                                    <option value="trancher_after_100_words"  <?php
                                if (get_option('vxl_trancherAt') == 'trancher_after_100_words') {
                                    print ' selected ';
                                }
                                ?> >   
<?php _e('Cut after approx 100 words', $this->plugin->name); ?></option> 
                                    <option value="trancher_after_50_words"  <?php
if (get_option('vxl_trancherAt') == 'trancher_after_50_words') {
    print ' selected ';
}
?> >   
<?php _e('Cut after about 50 words', $this->plugin->name); ?></option>  
                                    <option value="trancher_after_20_words"  <?php
if (get_option('vxl_trancherAt') == 'trancher_after_20_words') {
    print ' selected ';
}
?> >   
